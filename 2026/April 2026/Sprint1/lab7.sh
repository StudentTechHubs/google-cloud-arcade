...
echo "${CYAN_TEXT}${BOLD_TEXT}==============================================${RESET_FORMAT}"
echo
echo "${GREEN_TEXT}${BOLD_TEXT}Welcome to PerkVerse - Learn | Execute | Grow 🚀${RESET_FORMAT}"
echo "${BLUE_TEXT}${BOLD_TEXT}YouTube: https://www.youtube.com/@PerkVers${RESET_FORMAT}"
echo
...
gcloud compute scp perkverse.sh dev-instance:/tmp --project="$DEVSHELL_PROJECT_ID" --zone="$ZONE" --quiet

gcloud compute ssh dev-instance --project="$DEVSHELL_PROJECT_ID" --zone="$ZONE" --quiet --command='bash /tmp/perkverse.sh'
