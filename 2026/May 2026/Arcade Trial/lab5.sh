...
# User Inputs
read -p "${YELLOW_TEXT}${BOLD_TEXT}Enter CUSTOM_SECURITY_ROLE: ${RESET_FORMAT}" CUSTOM_SECURIY_ROLE
...
echo
echo "${MAGENTA_TEXT}${BOLD_TEXT}⚙️ Setting up your GCP environment...${RESET_FORMAT}"
echo
...
echo "${MAGENTA_TEXT}${BOLD_TEXT}Creating base service account...${RESET_FORMAT}"
gcloud iam service-accounts create orca-private-cluster-sa \
  --display-name "Orca Private Cluster Service Account"

echo "${MAGENTA_TEXT}${BOLD_TEXT}Creating custom IAM role...${RESET_FORMAT}"
...
# Task 4
echo "${MAGENTA_TEXT}${BOLD_TEXT}Creating GKE private cluster...${RESET_FORMAT}"
...
# Task 5
echo "${MAGENTA_TEXT}${BOLD_TEXT}Configuring jumphost & deploying app...${RESET_FORMAT}"
